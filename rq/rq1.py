...
def pag_NF(delta: np.ndarray) -> np.ndarray:
    ...
    ranking = np.empty((B, 2), dtype=int)

    # Max-heap via negative values: (-benefit, individual, level)
    heap = [(-delta[i, 0], i, 0) for i in range(n)]
    heapq.heapify(heap)

    for b in range(B):
        neg_val, i, l = heapq.heappop(heap)
        ranking[b] = (i, l)

        next_l = l + 1
        if next_l < L:
            heapq.heappush(heap, (-delta[i, next_l], i, next_l))
...
if __name__ == "__main__":
    ...
    # Heterogeneous strictly increasing marginal benefits
    # datasets/nonnegative/delta_000.csv, ...
    run_rq1_rankings(dataset_dir="datasets/heterogeneous_strictly_increasing", results_dir="resultsrq1sih")