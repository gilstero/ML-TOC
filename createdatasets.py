...
from heterogeneous_strictly_increasing import *
import os
...
HETERO_DIR = os.path.join(BASE_DIR, "heterogeneous_strictly_increasing")
...
# -----------------------------------------
# Heterogeneous Strictly Increasing Dataset
# -----------------------------------------
for r in range(R):
    _, delta, _ = generateheterogeneousstrictlyincreasing(
        n=n,
        L=L,
        sigma=0.5,
        seed=r,
    ...
    path = os.path.join(HETERO_DIR, f"delta_{r:03d}.csv")
    np.savetxt(path, delta, delimiter=",")
